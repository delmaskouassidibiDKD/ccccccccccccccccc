...
const COLLAB_KEY   = "@dkd:marche_collaborateurs";

type Conv = { id: string; name: string; initials: string; color: string; preview?: string; time?: string; role?: string };

const DEFAULT_CONTACTS: Conv[] = [
  { id: "mc1", name: "Kouassi Adjobi",    initials: "KA", color: "#22C55E", role: "Revendeur" },
  { id: "mc2", name: "Awa Traoré",        initials: "AT", color: "#3B82F6", role: "Fournisseur" },
  { id: "mc3", name: "Mamadou Coulibaly", initials: "MC", color: "#F59E0B", role: "Livreur" },
  { id: "mc4", name: "Fatoumata Bah",     initials: "FB", color: "#EC4899", role: "Caissière" },
];

export default function CollaborateurMarchePage() {
  ...
  useFocusEffect(useCallback(() => {
    const load = async () => {
      try {
        ...
        const extras: Conv[] = extraRaw ? (JSON.parse(extraRaw) as Conv[]).filter((c) => !existingIds.has(c.id)) : [];
  ...
  const handleAdd = async () => {
    ...
    const names = contacts.filter((c) => selectedIds.has(c.id)).map((c) => c.name).join(", ");
    ...
    <View style={[s.root, { backgroundColor: dynBG }]}>
      <View style={[s.header, { paddingTop: insets.top + 10, backgroundColor: dynHeader, borderBottomColor: dynBorder }]}>
        ...
        <View style={s.headerCenter}>
          ...
          <View>
            ...
            <Text style={[s.headerSub, { color: dynSub }]}>Mon Marché</Text>
          </View>
        </View>
      </View>

      <FlatList data={[]} renderItem={() => null} showsVerticalScrollIndicator={false}
        contentContainerStyle={{ paddingBottom: insets.bottom + 80 }}
        ListHeaderComponent={
          <View>
            {collaborateurs.length > 0 && (
              <View style={{ padding: 14, gap: 10 }}>
                <Text style={[s.sectionLabel, { color: isDark ? "#475569" : "#9CA3AF" }]}>MON ÉQUIPE ({collaborateurs.length})</Text>
                {collaborateurs.map((c) => (
                  <View key={c.id} style={[s.card, { backgroundColor: dynCARD, borderColor: ACCENT + "44" }]}>
                    <View style={[s.avatar, { backgroundColor: c.color + "22" }]}>
                      <Text style={[s.avatarText, { color: c.color }]}>{c.initials}</Text>
                    </View>
                    <View style={s.info}>
                      <Text style={[s.name, { color: dynText }]}>{c.name}</Text>
                      <View style={[s.badge, { backgroundColor: ACCENT + "18" }]}>
                        <Ionicons name="checkmark-circle" size={11} color={ACCENT} />
                        <Text style={[s.badgeText, { color: ACCENT }]}>Collaborateur actif</Text>
                      </View>
                    </View>
                    <TouchableOpacity style={[s.removeBtn, { backgroundColor: "#EF444418" }]} onPress={() => handleRemove(c.id)}>
            ...
            <View style={{ paddingHorizontal: 14, paddingBottom: 14, gap: 10 }}>
              <Text style={[s.sectionLabel, { color: isDark ? "#475569" : "#9CA3AF" }]}>CONTACTS ({disponibles.length})</Text>
              <Text style={[s.hint, { color: dynSub }]}>Sélectionnez les personnes à ajouter</Text>
              {disponibles.map((c) => {
                const selected = selectedIds.has(c.id);
                return (
                  <TouchableOpacity key={c.id}
                    style={[s.card, { backgroundColor: dynCARD, borderColor: selected ? ACCENT : dynBorder }, selected && { backgroundColor: ACCENT + "0A" }]}
                    onPress={() => toggleSelect(c.id)} activeOpacity={0.8}
                  >
                    <View style={[s.avatar, { backgroundColor: c.color + "22" }]}>
                      <Text style={[s.avatarText, { color: c.color }]}>{c.initials}</Text>
                    </View>
                    <View style={s.info}>
                      <Text style={[s.name, { color: dynText }]}>{c.name}</Text>
                      {c.role && <Text style={[s.role, { color: dynSub }]}>{c.role}</Text>}
                    </View>
                    <View style={[s.checkbox, { borderColor: selected ? ACCENT : dynBorder, backgroundColor: selected ? ACCENT : "transparent" }]}>
      ...
      {selectedIds.size > 0 && (
        <View style={[s.fab, { bottom: insets.bottom + 16 }]}>
          <TouchableOpacity style={[s.fabBtn, { backgroundColor: ACCENT }]} onPress={handleAdd} activeOpacity={0.9}>
...
const s = StyleSheet.create({
  ...
  card:   { flexDirection: "row", alignItems: "center", borderRadius: 14, borderWidth: 1, padding: 12, gap: 12 },
  avatar: { width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center" },
  avatarText: { fontFamily: "Poppins_700Bold", fontSize: 15 },
  info:   { flex: 1, gap: 3 },
  name:   { fontFamily: "Poppins_700Bold", fontSize: 14 },
  role:   { fontFamily: "Poppins_400Regular", fontSize: 12 },
