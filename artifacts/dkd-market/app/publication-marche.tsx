...
const METHODS: Method[] = [
  {
    ...
    badge: "NOUVEAU",
    route: "/quick-publish?context=marche",
  },
  {
    key: "normal_only",
    icon: "cart-outline",
    ...
    sub: "Formulaire complet : photos, description, catégorie, variantes et génération IA.",
    tags: ["Grand public", "Prix fixe", "Détail"],
    route: "/add-product?saleMode=normal_only",
  },
];

export default function PublicationMarchePage() {
    ...
    <View style={[s.root, { backgroundColor: dynBG }]}>
      ...
      <View style={[s.header, { paddingTop: insets.top + 10, backgroundColor: dynHeader, borderBottomColor: dynBorder }]}>
        ...
        <View style={s.headerCenter}>
          ...
          <View>
            <Text style={[s.headerTitle, { color: dynText }]}>Publier un produit</Text>
      ...
      >
        ...
        <View style={[s.introBox, { backgroundColor: ACCENT + "12", borderColor: ACCENT + "30" }]}>
          ...
          <Text style={[s.introText, { color: isDark ? "#86EFAC" : "#15803D" }]}>
            Mon Marché propose deux modes : une publication express pour aller vite, ou un formulaire complet pour plus de visibilité.
          </Text>
        </View>
        ...
        {/* What's next (only for full form) */}
        <View style={[s.nextBox, { backgroundColor: dynCARD, borderColor: dynBorder }]}>
          <Text style={[s.nextTitle, { color: dynText }]}>Avec le formulaire complet :</Text>
          {[
            { icon: "camera-outline",   text: "Jusqu'à 6 photos + 1 vidéo" },
            { icon: "pricetag-outline", text: "Prix, variantes et options" },
            { icon: "grid-outline",     text: "Catégorie et description détaillée" },
            { icon: "sparkles-outline", text: "Génération IA de la description" },
          ].map((item) => (
