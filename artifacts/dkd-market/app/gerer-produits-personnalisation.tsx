...
export default function GererProduitsPersonnalisationPage() {
  ...
  const TABS = [
    { key: "detail" as Tab, label: "Détail",  count: detail.length, icon: "cube-outline"   },
    ...
    <View style={[s.root, { backgroundColor: dynBG }]}>
      ...
      <View style={[s.statusBlock, { backgroundColor: dynHeader, borderBottomColor: dynBorder }]}>
        <View style={s.statusRow}>
          <View style={s.statusLeft}>
            ...
            <Text style={[s.statusLabel, { color: dynText }]}>{isActive ? "Boutique active" : "Boutique inactive"}</Text>
          </View>
      ...
      <View style={[s.searchWrap, { backgroundColor: dynBG, borderBottomColor: dynBorder }]}>
        <View style={[s.searchBox, { backgroundColor: isDark ? "#1E293B" : "#FFFFFF", borderColor: dynBorder }]}>
          ...
          <TextInput style={[s.searchInput, { color: dynText }]} placeholder={isEngros ? "Rechercher en gros…" : "Rechercher un produit…"} placeholderTextColor={dynSub} value={searchQuery} onChangeText={setSearchQuery} returnKeyType="search" />
