...
export default function DmImportePage() {
  ...
  const ACCENT    = "#A855F7";
  const contactColor = color ?? "#A855F7";
  ...
  const saveMessages = (msgs: Msg[]) => {
    AsyncStorage.setItem(storageKey, JSON.stringify(msgs));
  };
  ...
  const send = useCallback(() => {
    ...
    if (editingMsg) {
      ...
      setMessages((prev) => { const updated = [...prev, newMsg]; saveMessages(updated); return updated; });
  ...
  const handleDelete = (msg: Msg) => {
    ...
    setMessages((prev) => { const updated = prev.filter((m) => m.id !== msg.id); saveMessages(updated); return updated; });
    setContextMsg(null);
  };
    ...
    <View style={[s.root, { backgroundColor: dynBG }]}>
      ...
      <View style={[s.header, { paddingTop: insets.top + 10, backgroundColor: dynHeader, borderBottomColor: dynBorder }]}>
        ...
        <View style={s.headerInfo}>
          <Text style={[s.headerName, { color: dynText }]} numberOfLines={1}>{name ?? "Contact"}</Text>
        ...
        <View style={{ backgroundColor: dynHeader, borderTopWidth: 1, borderTopColor: dynBorder }}>
          ...
          {editingMsg && (
            <View style={[s.replyBanner, { backgroundColor: isDark ? "#1E293B" : "#F0F4FA", borderLeftColor: "#F59E0B" }]}>
              <Ionicons name="pencil-outline" size={14} color="#F59E0B" style={{ marginTop: 1 }} />
              <View style={{ flex: 1 }}>
              ...
              <TouchableOpacity onPress={cancelEdit}><Ionicons name="close" size={18} color={dynSub} /></TouchableOpacity>
            </View>
          )}
          ...
          {replyTo && !editingMsg && (
            <View style={[s.replyBanner, { backgroundColor: isDark ? "#1E293B" : "#F0F4FA", borderLeftColor: ACCENT }]}>
              <Ionicons name="return-down-forward-outline" size={14} color={ACCENT} style={{ marginTop: 1 }} />
              <View style={{ flex: 1 }}>
                <Text style={[s.replyBannerLabel, { color: ACCENT }]}>Réponse à {replyTo.isMe ? "vous" : (name ?? "contact")}</Text>
                <Text style={[s.replyBannerText, { color: dynSub }]} numberOfLines={1}>{replyTo.text}</Text>
              </View>
              <TouchableOpacity onPress={cancelReply}><Ionicons name="close" size={18} color={dynSub} /></TouchableOpacity>
            </View>
          )}
...
const s = StyleSheet.create({
  ...
  headerStatus: { fontFamily: "Poppins_400Regular", fontSize: 11 },
